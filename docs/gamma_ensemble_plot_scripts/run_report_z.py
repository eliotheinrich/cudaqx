...
# SHOTS_CAP caps every code's shots (smoke run); SHOTS forces an exact uniform count.
SHOTS_CAP = int(os.environ.get("SHOTS_CAP", "0"))
...
for slug, label, kind, params, shots, gamma0 in CODES:
    per_code = os.environ.get(f"SHOTS_{slug}")
    if per_code:
        shots = int(per_code)          # per-code override, e.g. SHOTS_surf_d9_r27=800
    elif SHOTS:
        shots = SHOTS
    elif SHOTS_CAP:
