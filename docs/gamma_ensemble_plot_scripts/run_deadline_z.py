...
DATA_ROOT = os.environ.get("QEC_DATA_ROOT", "/workspaces/qec-ensemble-test")
...
# Error rates are env-overridable so the same run can be repeated at raised p.
SURF_P = float(os.environ.get("SURF_P", "0.008"))
BB72_P = os.environ.get("BB72_P", "0.003")
BB144_P = os.environ.get("BB144_P", "0.001")
ENSEMBLE = [1, 2, 4, 8]
CRITERIA = [("FirstConv", "FirstConv", {}), ("NConv(5)", "NConv", {"stop_nconv": 5})]
GAMMA_DIST = [-0.24, 0.66]
GAMMA0 = 0.125                              # same gamma0 for every code
CODES = [
...
for slug, label, kind, params, gamma0 in CODES:
    nsh = int(os.environ.get(f"SHOTS_{slug}", SHOTS))   # per-code shot override
    t0 = time.time()
    if kind == "bb":
        H, er, syn = build_bb(params["tag"], params["p"], nsh); pstr = params["p"]
    else:
        H, er, syn = build_surface(params["d"], params["r"], params["p"], nsh); pstr = str(params["p"])
    det, err = H.shape
    store[f"{slug}__meta"] = np.array([label, f"{det}x{err}", pstr, NUM_SETS, MAX_ITER, PRE_ITER, gamma0, nsh], dtype=object)
    store[f"{slug}__crits"] = np.array([c[0] for c in CRITERIA], dtype=object)
    store[f"{slug}__ens"] = np.array(ENSEMBLE)
    print(f"\n=== {label} | DEM {det}x{err} | p={pstr} | {nsh} shots (built {time.time()-t0:.0f}s, elapsed {(time.time()-t_start)/60:.0f}m) ===", flush=True)
    for ci, (clab, criterion, extra) in enumerate(CRITERIA):
        for N in ENSEMBLE:
            ...
            wall = np.empty(nsh); it = np.empty(nsh, np.int32); cv = np.empty(nsh, bool)
            for i in range(nsh):
